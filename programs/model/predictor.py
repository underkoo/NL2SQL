...
class Predictor(nn.Module):
    def __init__(self, word_emb, N_word, N_h=200, N_depth=2,
            gpu=False, use_ca=True, use_cnn=False, use_num_cnn=False, use_col_cnn=False, use_op_cnn=False, filter_num=1, cnn_type=1, trainable_emb=False, agg=False, sel=False, cond=False, use_detach=False):
        ...
        if cond:
            self.cond_pred = CondPredictor(N_word, N_h, N_depth,
                self.max_col_num, self.max_tok_num, use_ca=use_ca, use_cnn=use_cnn, use_num_cnn=use_num_cnn, use_col_cnn=use_col_cnn, use_op_cnn=use_op_cnn, filter_num=filter_num, cnn_type=cnn_type, use_detach=use_detach, gpu=gpu)
    ...
    def loss(self, score, truth_num, pred_entry, gt_where):
        ...
        if pred_agg:
            ...
            data = torch.from_numpy(np.array(agg_truth, dtype='int64'))
            if self.gpu:
        ...
        if pred_sel:
            ...
            data = torch.from_numpy(np.array(sel_truth, dtype='int64'))
            if self.gpu:
        ...
        if pred_cond:
            ...
            data = torch.from_numpy(np.array(cond_num_truth, dtype='int64'))
            if self.gpu:
            ...
            for b in range(len(truth_num)):
                if len(truth_num[b][4]) == 0:
                    continue
                data = torch.from_numpy(np.array(truth_num[b][4], dtype='int64'))
                if self.gpu:
            ...
            for b in range(len(gt_where)):
                for idx in range(len(gt_where[b])):
                    ...
                    if len(cond_str_truth) == 1:
                        continue
                    data = torch.from_numpy(np.array(cond_str_truth[1:], dtype='int64'))
                    if self.gpu:
